...
describe('AssistantMessage', () => {
  it('exposes distinct, named user and assistant turns to assistive technology', () => {
    render(<>{sampleMessages.map((message) => <AssistantMessage key={message.id} message={message} onRetry={vi.fn()} onCitationClick={vi.fn()} />)}</>);

    expect(within(screen.getByRole('article', { name: 'You' })).getByText(sampleMessages[0].content)).toBeVisible();
    expect(within(screen.getByRole('article', { name: 'Assistant' })).getByText(sampleMessages[1].content)).toBeVisible();
  });
  ...
  it('names each source by kind and title and returns the selected citation', async () => {
    ...
    render(<AssistantMessage message={{ ...sampleMessages[1], citations: sampleCitations }} onRetry={vi.fn()} onCitationClick={onCitationClick} />);
    ...
    for (const [index, citation] of sampleCitations.entries()) {
      ...
      await user.click(sources.getByRole('button', { name: `${kind}: ${citation.title}` }));

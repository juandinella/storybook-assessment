...
function renderDemo(scenario: DemoScenario) {
  ...
  const panel = within(screen.getByRole('region', { name: 'Report assistant' }));
  return {
    user,
...
// Control the stream without trapping Testing Library's async timeout scheduling.
...
describe('AssistantDemo', () => {
  it('sends and clears the draft, allows editing during streaming, and retains the next draft on completion', async () => {
    ...
    expect(within(thread.getByRole('article', { name: 'You' })).getByText(sampleSuggestions[0], { exact: true })).toBeVisible();
    expect(input).toHaveValue('');
    expect(input).toHaveFocus();
    const answer = thread.getByRole('article', { name: 'Assistant' });
    expect(within(answer).getByText('Preparing', { exact: true })).toBeVisible();
    ...
    expect(within(answer).getByText(sampleMessages[1].content, { exact: true })).toBeVisible();
  ...
  it.each([0, 12])('Stop after %i characters freezes the answer without submitting or clearing the next draft', async (characters) => {
    const { user, input, thread, announcement } = renderDemo('streaming');
    const answer = thread.getByRole('article', { name: 'Assistant' });
    const id = answer.getAttribute('data-message-id');
    await act(async () => { await vi.advanceTimersByTimeAsync(55 * characters); });
    const partial = sampleMessages[1].content.slice(0, characters);
    if (characters) expect(within(answer).getByText(partial, { exact: true })).toBeVisible();
    else expect(within(answer).getByText('Preparing', { exact: true })).toBeVisible();
    ...
    expect(within(answer).queryByText('Preparing', { exact: true })).not.toBeInTheDocument();
    expect(within(answer).getByText(characters
      ? 'Response stopped. This answer is incomplete.'
      : 'Response stopped. No answer was generated.')).toBeVisible();
    expect(announcement).toHaveTextContent('Response stopped.');
  ...
  it('retries the same failed ID in place without duplicating turns or changing the draft', async () => {
    ...
    expect(announcement).toHaveTextContent('Response failed. Retry is available.');
    await user.type(input, sampleSuggestions[2]);
    ...
    expect(answer).toHaveAttribute('data-message-id', errorMessage.id);
    expect(within(answer).getByText('Preparing', { exact: true })).toBeVisible();
    expect(within(answer).queryByText(errorMessage.content)).not.toBeInTheDocument();
    expect(within(answer).queryByRole('button', { name: 'Retry' })).not.toBeInTheDocument();
  ...
  it('retries an older failed turn after a later answer, announcing its transitions without rereading streamed characters', async () => {
    ...
    const laterAnswer = thread.getAllByRole('article', { name: 'Assistant' })[1];
    ...
    expect(within(olderAnswer).getByText('Preparing', { exact: true })).toBeVisible();
    expect(announcement).toHaveTextContent('Preparing response.');
    await act(async () => { await vi.advanceTimersByTimeAsync(55 * 12); });
    expect(within(olderAnswer).getByText(sampleMessages[1].content.slice(0, 12), { exact: true })).toBeVisible();
    expect(announcement).toHaveTextContent('Preparing response.');
    expect(laterAnswer.textContent).toBe(laterContent);
    await act(async () => { await vi.runAllTimersAsync(); });

    expect(announcement).toHaveTextContent('Response complete.');
    expect(thread.getAllByRole('article').map((turn) => turn.getAttribute('data-message-id'))).toEqual(ids);
    expect(within(olderAnswer).getByText(sampleMessages[1].content, { exact: true })).toBeVisible();
    expect(laterAnswer.textContent).toBe(laterContent);
    expect(input).toHaveValue(sampleSuggestions[2]);
    expect(input).toHaveFocus();
  });
  ...
  it('immediately submits the exact suggestion, clears an existing draft, and restores composer focus', async () => {
    ...
    expect(thread.getAllByRole('article')).toHaveLength(2);
    expect(within(thread.getByRole('article', { name: 'You' })).getByText(sampleSuggestions[0], { exact: true })).toBeVisible();
    expect(thread.queryByText(sampleSuggestions[2], { exact: true })).not.toBeInTheDocument();
    expect(within(thread.getByRole('article', { name: 'Assistant' })).getByText('Preparing', { exact: true })).toBeVisible();
